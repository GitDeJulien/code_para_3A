...
## Cas 1 (Stationnaire)
 ...
 - Conditions de bord (key 1) : $g=0$ et $\mahtcal{h}=0$
 - Solution exacte : $u_{exact} = x(1-x)y(1-y)$

## Cas 2 (Stationnaire)

 - Terme source (key 2) : $f = \sin(x) + \cos(y)$ 
 - Conditions de bord (key 3) : $g=\sin(x) + \cos(y)$ et $\mahtcal{h} =\sin(x) + \cos(y)$
...
## Cas 3 (Instationnaire périodique)
 ...
 - Conditions de bord (key 2) : $g=0$ et $\mahtcal{h} =1$
